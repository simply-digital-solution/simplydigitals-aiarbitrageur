...
export default function PortfolioSummary() {
  ...
  const fetchPortfolio = async () => {
    ...
    try {
      ...
      console.error('Error fetching portfolio:', _err);
      // Mock data for demo
      const mockPortfolio = {
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      <div className="rounded-2xl border border-slate-700 bg-slate-950/60 p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Open Positions</p>
          <span className="text-sm font-medium text-sky-400">{positions.length}</span>
        </div>
            ...
            {positions.map((pos) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">${(pos.avg_entry_price || 0).toFixed(2)}</span>
                  <span className={pos.unrealized_pl >= 0 ? 'text-emerald-400' : 'text-rose-400'}>
                    {pos.unrealized_pl >= 0 ? '+' : ''}${(pos.unrealized_pl || 0).toFixed(2)}
                  </span>
                </div>
              </div>
            ))}
