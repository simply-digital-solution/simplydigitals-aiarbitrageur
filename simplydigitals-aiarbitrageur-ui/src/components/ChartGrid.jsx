      ...
      for (const symbol of selectedSymbols) {
        try {
          const resp = await axios.get(
            `${API_BASE_URL}/prices/${symbol}/intraday-1min?limit=60`,
          );
  ...
  const appendChartData = async () => {
    ...
    try {
      for (const symbol of selectedSymbols) {
        try {
          const resp = await axios.get(
            `${API_BASE_URL}/prices/${symbol}/intraday-1min?limit=5`,
          );
          ...
          setChartsData((current) => {
            ...
            const lastTs =
              existing.length > 0 ? existing[existing.length - 1].ts : null;
    ...
    <div className="space-y-6">
      ...
      {selectedSymbols.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-700 p-12 text-center">
          <p className="text-slate-400">
            Select tickers from the watchlist to view charts
          </p>
        </div>
      ) : loading ? (
        ...
        <div className={`grid gap-4 ${getGridClass()}`}>
          {selectedSymbols.map((symbol, idx) => {
            ...
            const maxPrice =
              data.length > 0 ? Math.max(...data.map((d) => d.high)) : 0;
            const latestClose =
              data.length > 0 ? data[data.length - 1].close : 0;
            const change =
              data.length > 1
                ? (((latestClose - data[0].open) / data[0].open) * 100).toFixed(
                    2,
                  )
                : 0;
      ...
      {selectedSymbols.length > 0 && (
        <div className="rounded-lg bg-slate-900/50 border border-slate-700 px-4 py-3 text-xs text-slate-400">
          ...
          {selectedSymbols.length > 1 ? "s" : ""} • Data updates every 60
          seconds
        </div>
      )}
    </div>
